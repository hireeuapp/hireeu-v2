...
Respond ONLY with JSON — no markdown, no explanation:
{
  ...
  const qualified = allScored.filter((j) => j.match >= minScore);
...
export default async function handler(req, res) {
  ...
  const qualified = await matchAndScoreJobs(
    { summary, skills: skills || [], yearsExperience: yearsExperience || 0 },
    jobs,
    MIN_SCORE
  );

  console.log(
    `match-and-score: ${jobs.length} in → ${qualified.length} above ${MIN_SCORE}% threshold`
  );
