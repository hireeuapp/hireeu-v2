...
function isEnglishJob(job) {
  const sample = `${job.title} ${job.description}`.slice(0, 600).toLowerCase();
  if (sample.length < 40) return true;
  if (/[àáâãäåæçèéêëìíîïðñòóôõöùúûüýþßœ]{3,}/i.test(sample)) return false;
  const hits = (sample.match(/\b(the|and|for|with|you|our|this|will|have|are|we|your|that|from|an?)\b/gi) || []).length;
...
function isPoland(job) {
  ...
  return loc.includes('poland') || loc.includes('warszawa') || loc.includes('warsaw') || loc.includes('kraków') || loc.includes('krakow') || loc.includes('wrocław') || loc.includes('wroclaw') || loc.includes('gdańsk') || loc.includes('gdansk') || loc.includes('poznań') || loc.includes('poznan') || loc.includes('łódź') || loc.includes('lodz');
}
...
async function fetchJSearch(queries, apiKey) {
  const fetches = queries.map(async (q) => {
    try {
      ...
      const r = await fetch(url.toString(), { headers: { 'X-RapidAPI-Key': apiKey, 'X-RapidAPI-Host': 'jsearch.p.rapidapi.com' }, signal: AbortSignal.timeout(12000) });
      if (!r.ok) return [];
      const d = await r.json();
      return (d.data || []).slice(0, 10).map(j => ({ id: 'js_' + (j.job_id || Math.random()), title: j.job_title || '', company: j.employer_name || '', location: [j.job_city, j.job_country].filter(Boolean).join(', ') || 'Poland', description: (j.job_description || '').slice(0, 1000), applyUrl: j.job_apply_link || j.job_google_link || '', postedAt: j.job_posted_at_datetime_utc || null, source: 'JSearch', isRemote: j.job_is_remote || false }));
    } catch { return []; }
  });
...
export async function searchJobsByRole(role) {
  ...
  if (!jsearchKey && !adzunaId) throw new Error('No API keys configured.');
  const { queries, qaIntent } = expandRole(role);
  const [jsearchJobs, adzunaJobs] = await Promise.all([jsearchKey ? fetchJSearch(queries, jsearchKey) : Promise.resolve([]), adzunaId ? fetchAdzuna(queries, adzunaId, adzunaKey) : Promise.resolve([])]);
  const raw = [...jsearchJobs, ...adzunaJobs];
  let clean = dedupe(raw).filter(isEnglishJob).filter(j => !isBlocked(j));
  if (qaIntent) clean = clean.filter(jobMatchesQaIntent);
  clean.sort((a, b) => { const ap = isPoland(a) ? 1 : 0; const bp = isPoland(b) ? 1 : 0; if (ap !== bp) return bp - ap; const ad = a.postedAt ? new Date(a.postedAt).getTime() : 0; const bd = b.postedAt ? new Date(b.postedAt).getTime() : 0; return bd - ad; });
