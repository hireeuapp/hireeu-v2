...
function isEnglishJob(job) {
  ...
  // Reject on high Polish character density
...
async function fetchAdzuna(queries, appId, appKey) {
  ...
  for (const q of queries.slice(0, 4)) {
    fetches.push((async () => {
      try {
        const url = new URL('https://api.adzuna.com/v1/api/jobs/pl/search/1');
...
export async function searchJobsByRole(role, options = { englishOnly: false }) {
  ...
  const [jsearchRes, adzunaJobs, scraperJobs] = await Promise.all([
    jsearchKey ? fetchJSearch(queries, jsearchKey) : Promise.resolve({ data: [], errors: [] }),
  ...
  // Sort: Poland-based jobs first, then by recency
  clean.sort((a, b) => {
