...
// ── City aliases ──────────────────────────────────────────────────────────────
// Maps common spellings to all variants job APIs might return.
const CITY_ALIASES = {
...
/**
 * Parse a location string like "Berlin, Germany", "Warsaw", or "Germany"
 * and return { adzunaCode, jsearchPhrase, cityAliases }.
 * cityAliases is empty when only a country is specified (no city filter needed).
 */
function resolveLocation(preferredLocation) {
  ...
  // Full string match against country map (e.g. "united kingdom")
  if (COUNTRY_MAP[raw]) {
    const { code, phrase } = COUNTRY_MAP[raw];
    return { adzunaCode: code, jsearchPhrase: phrase, cityAliases: [] };
  }

  // Token match: separate country tokens from city tokens
  let countryCode = null, countryPhrase = null;
  const cityTokens = [];

  for (const token of tokens) {
    if (COUNTRY_MAP[token]) {
      countryCode = COUNTRY_MAP[token].code;
      countryPhrase = COUNTRY_MAP[token].phrase;
    } else {
      cityTokens.push(token);
    }
  }

  // If we found a country token
  if (countryCode) {
    // Also look for a city among the remaining tokens
    const cityAliases = cityTokens.flatMap(t => CITY_ALIASES[t] || (CITY_TO_COUNTRY[t] ? [t] : []));
    return { adzunaCode: countryCode, jsearchPhrase: countryPhrase, cityAliases };
  }

  // No country token — check if any token is a known city
  for (const token of tokens) {
    if (CITY_TO_COUNTRY[token]) {
      const code = CITY_TO_COUNTRY[token];
      const phrase = COUNTRY_CODE_TO_PHRASE[code];
      const cityAliases = CITY_ALIASES[token] || [token];
      return { adzunaCode: code, jsearchPhrase: phrase, cityAliases };
    }
  }

  // Fallback: unknown location, default to Poland, use raw string as city alias
  return { adzunaCode: 'pl', jsearchPhrase: 'Poland', cityAliases: [raw] };
}

function locationMatchesCity(jobLocation, cityAliases) {
  if (!cityAliases.length) return true;
...
/**
 * Language check: for Poland jobs, we apply a stricter English filter.
 * For other countries, job boards already serve English-language listings,
 * so we only reject on high Polish character density (common false-positive).
 */
function isEnglishJob(job, strictMode = false) {
  const title = (job.title || '').toLowerCase();
  const desc = (job.description || '').toLowerCase();
  const text = `${title} ${desc}`.slice(0, 1500);

  // Always reject on high Polish character density
  const polishChars = (text.match(/[ąćęłńóśźż]/gi) || []).length;
  if (polishChars > 15) return false;

  // In strict mode (Poland), also require English stop-word presence
  if (strictMode) {
...
async function fetchPracujScraper(role, baseUrl) {
  if (!baseUrl) return [];
  try {
    fetch(`${baseUrl.replace(/\/$/, '')}/scrape-async?role=${encodeURIComponent(role)}`).catch(() => {});
    const r = await fetch(`${baseUrl.replace(/\/$/, '')}/scrape-cached?role=${encodeURIComponent(role)}`, {
...
export async function searchJobsByRole(role, options = {}) {
  ...
  const isPolandSearch = adzunaCode === 'pl';
  ...
  const [jsearchRes, adzunaJobs, scraperJobs] = await Promise.all([
    ...
    (scraperUrl && isPolandSearch) ? fetchPracujScraper(role, scraperUrl) : Promise.resolve([]),
  ]);
  ...
  // Language filtering: strict mode only for Poland (where Polish-language jobs are common)
  const beforeLang = clean.length;
  clean = clean.filter(j => {
    if (!isEnglishJob(j, isPolandSearch)) return false;
  ...
  // City filter: only apply if the user specified a city (not just a country)
  ...
  // Sort: preferred city/country first, then by recency
  const preferredAliases = cityAliases.length > 0
    ? cityAliases
    : [jsearchPhrase.toLowerCase()]; // e.g. 'germany', 'poland'

  clean.sort((a, b) => {
    const ap = locationMatchesCity(a.location, preferredAliases) ? 1 : 0;
    const bp = locationMatchesCity(b.location, preferredAliases) ? 1 : 0;
