...
function hasProhibitedLanguages(job) {
  const desc = (job.description || '').toLowerCase();
  // Phrases that indicate Polish is required even if the job post is in English
  const polishRequired = /\b(must speak|required|vorausgesetzt|wymagan[ay]|znajomość|mówiący|płynny|at least)\b.*\b(polish|polski|polskiego|polsku)\b/i;
  const germanRequired = /\b(must speak|required|vorausgesetzt|wymagan[ay]|znajomość|at least)\b.*\b(german|niemiecki|deutsch)\b/i;
  
  // Special case for language levels
  const levels = /\b(polish|german|polski|deutsch)\b.*\b(c1|c2|fluent|native|profiency)\b/i;

  return polishRequired.test(desc) || germanRequired.test(desc) || levels.test(desc);
}
