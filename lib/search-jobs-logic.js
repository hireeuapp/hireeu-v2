...
function isEnglishJob(job) {
  // If it's a Polish market scraper result, we are a bit more lenient
  if (job.source === 'Pracuj.pl' || job.source === 'NoFluffJobs') {
    const title = (job.title || '').toLowerCase();
    // English keywords in titles common in Poland tech
    if (/\b(developer|engineer|tester|qa|manager|specialist|analyst|intern|backend|frontend|fullstack|devops)\b/i.test(title)) return true;
  }
...
export async function searchJobsByRole(role) {
  ...
  const [jsearchJobs, adzunaJobs, scraperJobs] = await Promise.all([
  ...
  const raw = [...jsearchJobs, ...adzunaJobs, ...scraperJobs];
