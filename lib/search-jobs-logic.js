...
function isEnglishJob(job, forceStrict = false) {
  const title = (job.title || '').toLowerCase();
  const desc = (job.description || '').toLowerCase();
  const text = `${title} ${desc}`.slice(0, 1500);
  
  // Polish character density check (more robust than just keywords)
...
function hasProhibitedLanguages(job) {
  ...
  // 1. Core keywords for languages and requirement indicators
  const Langs = '(polish|polski|polskiego|polsku|german|niemiecki|deutsch)';
  const Flags = '(must speak|required|vorausgesetzt|wymagan[ay]|znajomość|mówiący|płynny|at least|fluency|fluent|knowledge|level|level|płynna|komunikatywna|proficient)';
  const Levels = '(b2|c1|c2|fluent|native|profiency|advanced)';

  // 2. Look for [Flag] ... [Lang] or [Lang] ... [Flag/Level] within a reasonable proximity (60 chars)
  // Also catch "Polish C1" or "Polish-C1" (short distance)
  ...
  // 3. Catch specific phrases like "English and Polish" or "Polish and English"
  const together = /\b(english|polish)\s+and\s+(polish|english)\b/i;

  return pattern1.test(desc) || pattern2.test(desc) || levelShorthand.test(desc) || (together.test(desc) && /\brequired\b/i.test(desc));
}
...
export async function searchJobsByRole(role, options = { englishOnly: false }) {
