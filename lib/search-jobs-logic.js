...
export async function searchJobsByRole(role) {
  ...
  console.log(`[search] Initiating search for: "${role}"`);
  console.log(`[search] Sources detected: ${[jsearchKey?'JS':'', adzunaId?'AD':'', scraperUrl?'SC':''].filter(Boolean).join(', ')}`);
  ...
  console.log(`[search] Raw results found: ${raw.length}`);

  let clean = dedupe(raw);
  const beforeLang = clean.length;
  clean = clean.filter(isEnglishJob);
  console.log(`[search] Filtered by Language: ${beforeLang} -> ${clean.length}`);

  clean = clean.filter(j => !isBlocked(j));
  
  if (qaIntent) {
    const beforeQa = clean.length;
    clean = clean.filter(jobMatchesQaIntent);
    console.log(`[search] Filtered by QA Intent: ${beforeQa} -> ${clean.length}`);
  }
  ...
  console.log(`[search] Final job count returned: ${clean.length}`);
