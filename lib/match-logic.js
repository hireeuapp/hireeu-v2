...
function calculateScore(candidate, jobFacts) {
  ...
  return { score: total, reason, missingSkills };
}
...
PROFILE: ${summary} Skills: ${skills.join(', ')} Exp: ${yearsExperience}
For each job extract: yearsRequired, requiredSkills, criticalSkills, seniorityMatch ("match"|"stretch"|"overqualified"), fieldMatch (true|false|null).
JOBS: ${jobList}
Response JSON only: {"jobs": [{"index": 0, "yearsRequired": 3, "requiredSkills": [], "criticalSkills": [], "seniorityMatch": "match", "fieldMatch": true}]}`;
    try {
    ...
    } catch { batch.forEach(j => allScored.push({ ...j, match: 50, reason: 'Analysis failed.', missingSkills: [] })); }
  }
