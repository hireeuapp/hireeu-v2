...
function calculateScore(candidate, jobFacts) {
  ...
  let seniorScore = 0;
  if (seniority === 'match') { seniorScore = W_SENIORITY; strengths.push('seniority match'); }
  ...
  // Role mismatch penalty (e.g. Driver vs Developer)
...
export async function matchAndScoreJobs(candidate, jobs, minScore = MIN_SCORE) {
  ...
  for (let start = 0; start < jobs.length; start += BATCH) {
    ...
    const prompt = `Extract facts for job-matching. Respond ONLY with JSON — no preamble.
PROFILE: ${candidate.primaryRole || ''} - ${summary} Skills: ${skills.join(', ')} Exp: ${yearsExperience}
For each job extract: yearsRequired, requiredSkills, criticalSkills, seniorityMatch ("match"|"stretch"|"overqualified"), roleMatchScore (0.0–1.0), fieldMatch (true|false|null).
JOBS: ${jobList}
